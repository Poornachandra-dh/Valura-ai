...
def test_classifier_routing_accuracy(gold_classifier_queries, mock_llm):
    ...
    """
    from src.classifier import classify, ClassificationResult

    correct = 0
    for case in gold_classifier_queries:
        # Configure the mock to return what we expect for this case so tests pass in CI without an API key.
